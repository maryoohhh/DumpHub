...
const ForgotPassword = ({ navigation }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const { data } = await axios.post("https://urban-xylophone-4q6pqg49j7pf7xv6-8000.app.github.dev/api/forgot-password",  { email });
    ...
    const handlePasswordReset = async () => {
        try {
            const { data } = await axios.post("https://urban-xylophone-4q6pqg49j7pf7xv6-8000.app.github.dev/api/reset-password", { email, resetCode, password });
            if (data.error) alert(data.error);
            else {
        ...
        <KeyboardAwareScrollView contentContainerStyle={styles.container}>
            <View style={{ marginVertical: 100 }}>
                ...
                <View style={{ marginHorizontal: 24 }}>
                    ...
                    <TextInput style={styles.signupText} value={email} onChangeText={text => setEmail(text)} autoCompleteType="email" keyboardType="email-address" />
                </View>
                {visible && (
                    <>
                        <View style={{ marginHorizontal: 24 }}>
